...
function CardData({ cardExp, settings }) {
    ...
    return (
        <div>
            {!cardExp.isActive 
            ? <p className={style.desactivCard}>{ stringsPatterns['Card not activated, please contact card service!'][lenguage] }</p> 
            : (cardExp.tickets.length 
                ...
                <div className={style.infoBlock}>
                    ...
                    <div><span className={style.bottomSpan}>{ stringsPatterns['Note: it can take several days for these data to be updated since you loaded the card'][lenguage] }</span></div>
                </div>
                : null) 
            }
            {(!cardExp.tickets.length && cardExp.isActive) ? 
                <div style={{textAlign: "center"}}>
                    <span>{ stringsPatterns['The card is active, but there is no actual data on the purchased tickets '][lenguage] }</span>
                    <strong>({cardExp.cardCode})</strong>
                </div> 
            : null}
            <hr /> 
            <div className={style.divImageTarjeta}>
                    <div>{ stringsPatterns['Please enter the digits of your card as shown in the example!'][lenguage] }</div>
                    <div><img src={Tarjeta} alt='Example card' /></div>
            </div>
        </div>   
    )
}
