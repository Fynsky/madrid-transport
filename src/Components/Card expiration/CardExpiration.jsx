...
const CardExpiration = props => {
  ...
  const classNameCardExpirationMain = classNames(style.cardExpiration, {
  ...
  return (
    <div className={classNameCardExpirationMain}>
      ...
      <form className={style.form} onSubmit={e => onSubmitHandler(e)}>
        ...
        />
        <button ref={buttonRef} className="button blue small">
          {stringsPatterns['Find'][lenguage]}
        </button>
      </form>
      <div>
        {loading ? (
          <div>
            <Loader /> <p>{stringsPatterns['Loading...'][lenguage]}</p>
          </div>
        ) : null}
        {Object.values(cardExpData).length && !Object.values(error).length && !loading ? (
          ...
          <div className={style.divImageTarjeta}>
            <div>
              {
                stringsPatterns[
                  'Check when your subscription expires. Please enter the digits of your card as shown in the example!'
                ][lenguage]
              }
            </div>
            <div>
              <img src={Tarjeta} alt="examle how to enter the digits" />
            </div>
          </div>
        ) : null}
