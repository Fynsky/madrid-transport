...
function BusInput({alert, setAlert, inputValue, setInputValue, visitedStops, showFavor, toogleFavoriteHandler, 
                settings, allStopsJson, searchResults, getAllStops, setValueOfStop, sizeUserSreen }) {
    ...
    const classNameInputDivMain = classNames(
        style.inputDiv,
        {
         [style.colorTheme]: settings.screenTheme === 'color',
         [style.darkTheme]: settings.screenTheme === 'dark',
    ...
    return (
        <div className={classNameMain}>
            ...
            <div className={classNameInputDivMain}>
                <div className={style.starDiv}>
                    <img src={showFavor ? (settings.screenTheme === 'dark' ? starEmptyWhite : starEmptyBlack) : star} 
                        onClick={e => toogleFavoriteHandler(e)} className={classNameStarImage} alt='star favorites'/>
                </div>
                <form onSubmit={(e) => onSubmitBusStop(e)}> 
                    <input placeholder={sizeUserSreen === 'big' ? stringsPatterns['put your bus stop'][lenguage] : '...'}  
                           onChange={(e) => handlerInputValue(e)} type='text' value={inputValue} 
                           title={ stringsPatterns['put your bus stop'][lenguage] }  ref={refInput}/>
                    <select className={style.selectForm} onChange={(e) => handlerSelectChange(e)} >
                        {visStops.map((x,y) => <option key={y} >{x}</option>)}
                    </select>        
            ...
            ? (/^[\d-:]*$/.test(inputValue) 
                ? <p className={style.unfortunatelyParagraph}>{ stringsPatterns['If you are looking for a stop by its number, then click the "Find" button'][lenguage] }</p>
                : <p className={style.unfortunatelyParagraph}>{ stringsPatterns['Unfortunately, no such stops were found.'][lenguage] }</p>
              ) 
            : null)}
        </div>
    )
