...
function InputSearchResults({ searchResults, inputValue, settings }) {
    ...
    const classNameInputSearchResultMain = classNames(
        style.main,
        {
    ...
    return (
        <div className={classNameInputSearchResultMain}>
            {(searchResults) ? <div className={style.dataFindDiv}>{ stringsPatterns['Found data:'][lenguage] }</div> : null}
            {searchResults && searchResultsPortion.map(item => {
                return <Stop item={item} key={item[0]} settings={settings}/>
            })}
            {searchResultsRestLength ? 
            <div className={style.howMoreStopsLeftDiv}>
                <span>
                    { stringsPatterns['There are more'][lenguage] } {searchResultsRestLength} { stringsPatterns['stops'][lenguage] }
                </span>
                <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={(e) => handlerButtonClick(e)} className='red button'> 
                    {searchResultsRestLength > 10 ? stringsPatterns['Next 10'][lenguage] : `Next ${searchResultsRestLength}`}
                </button>
            </div> : null}
        </div>
    )
