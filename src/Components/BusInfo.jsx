...
function BusInfo(props) {
  ...
  const classNameBusInfoMain = classNames(style.mainBusInfoPage, {
  ...
  return (
    <div className={classNameBusInfoMain}>
      {error ? (
        <div className={style.errorParagraph}>
          {error.toString()}
          <div>{stringsPatterns['No data was received from the server...'][lenguage]}</div>
        </div>
      ) : null}
      {loading && !error ? (
        <div className={style.loader}>
          <Loader /> {stringsPatterns['Loading...'][lenguage]}{' '}
        </div>
      ) : null}
      ...
      {Object.keys(data).length ? (
        <div>
          <div className={style.stopNameDiv}>
            <div>
              {stringsPatterns['Stop code:'][lenguage]}
              <span>
                <strong>{data.stopNumber}</strong>
                <img
                  alt="data refresh icon"
                  onClick={() => props.getDataFromStopNumber(stopNumberUrl)}
                  src={RefreshBut}
            ...
            <div>
              <p>{stringsPatterns['Stop name:'][lenguage]}</p>
              <strong>{data.stopName}</strong>
            </div>
            <div>
              {stringsPatterns['Stop type:'][lenguage]}
          ...
          />
          {groups.length ? (
            groups.map(group => <TransportItems data={group} key={group[0]} settings={settings} />)
          ) : (
            <div style={{ textAlign: 'center' }}>====Actualmente sin datos====</div>
          )}

          <div className={style.dataAlertMessage}>{data.alertMessage && data.alertMessage}</div>
        </div>
      ) : null}
    </div>
  )
...
function getDeparturesGroups(lines) {
  ...
  lines.forEach(departure => {
    if (groups[departure.lineNumber]) {
      ...
      groups[departure.lineNumber] = [departure]
    }
  })
