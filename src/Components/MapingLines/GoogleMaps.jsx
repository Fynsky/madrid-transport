...
const MyMapComponent = compose(
  withProps({
    googleMapURL:"https://maps.googleapis.com/maps/api/js?key=AIzaSyCkmmOUieIPx8iN6dc63tNE5VIXiqGpJdo&v=3.exp&libraries=geometry,drawing,places",
      ...
      <pinch-zoom>
        <div>
          { props.searchResultsForMaping.map(item => 
            <Marker item={item} position={{ lat: (item[2]), lng: item[3] }} key={item[0]} 
...
const GoogleMaps = React.memo((props) => {
  ...
  return (
    <MyMapComponent
      ...
      onMarkerClick={handleToggleOpen} isOpen={openInfoWindowMarkerId} history={history} 
