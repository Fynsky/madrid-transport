        ...
        return (
            <div className={style[`${this.props.settings.screenTheme}`]}>
                <button type='button' className='button red small' onClick={(e) => this.openModal(e)}>
                    {this.props.isFavorStop ? `${del[this.state.lenguage]}` 
                    : `${add[[this.state.lenguage]]}`}
                </button>

                {this.state.isOpen  && (<div className='modal'>
                    <div className={style.modalBody}>
                        {this.state.isFavorStop ? <h1 className={style.headH1}>{removeStop[this.state.lenguage]}</h1> : <h1 className={style.headH1}>{addStop[this.state.lenguage]}</h1>}
                        {this.state.isFavorStop ? <p className={style.paragraph}>{areYouShureRemove[this.state.lenguage]}</p> :
                                     <p className={style.paragraph}>{areYouShureAdd[this.state.lenguage]}</p>}
                        <button type='button' className='button red small' onClick={(e) => this.closeModal(e)}>{cancel[this.state.lenguage]}</button>
