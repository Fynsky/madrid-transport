...
function Header({ busImageRef, menuImageRef, showNav }) {
    ...
    const classNameHeaderMain = classNames(
        style.headerMain,
        {
    ...
    const classNameBusImage = classNames (
        'headerBusImage',
        {[style.busImage]: true}
    )
    const classNameMenuImage = classNames (
        'headerBusImage',
        {
    ...
    return (
        <div className={classNameHeaderMain}>
            <div className={style.divImagesMenuAndBus}>
                <img className={classNameMenuImage} ref={menuImageRef} alt='icon menu'
                    src={settings.screenTheme === 'color' ? menuBlack : menuWhite} />
                <img className={classNameBusImage} alt='main icon bus'
                    src={settings.screenTheme === 'color' ? bus2 
                    :(settings.screenTheme === 'dark' ? bus1 : bus3) } ref={busImageRef}/>
            </div>
            <div className={style.textDiv}>
                <strong>{ stringsPatterns['Transport Madrid'][lenguage] }</strong>
            </div>
            <Link to='/'><img src={houseBlack} className={style.houseBut} alt='icon house'></img></Link>
        </div>
    )
}
