import {getDataAPI, getArrayAllBusStops} from '../api/api'
    ...
    switch (action.type){
        ...
        case SET_INPUT_VALUE:
          return {
            ...
            searchResults: state.allStopsJson ? state.allStopsJson.filter(item => item[1].toUpperCase().includes(action.inputValue.toUpperCase())) : []
          } 
...
export const setSearchResultsForMapingActionCreator = (lineNumber, transportType) => ({type: SET_SEARCH_RESULTS_FOR_MAPING, lineNumber, transportType})
...
function addToArrayIfNotPresent(originalArray, newElement) {
